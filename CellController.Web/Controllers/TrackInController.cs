...
{
    public class TrackInController : Controller
    {
        ...
        [HttpGet]
        public JsonResult GetTrackInEquipment(string LotNo, string UserID, string Location)
        {
            TrackEquipment equipment = new TrackEquipment();
            equipment = HttpHandler.GetTrackInEquipment(LotNo, UserID, Location);
        ...
        //function for getting data from RMS (recipes)
        [HttpGet]
        ...
        [HttpPost]
        public string TrackIn(string UserID, string LotNo, string Equipment, int TrackInQty, string Comment, string Location, List<string> lstProgram, List<string> lstEquip)
        {
            var result = HttpHandler.trackIn(UserID, LotNo, Equipment, TrackInQty, Comment, Location, lstProgram, lstEquip);
            return result;
        }
        ...
        [HttpGet]
        public JsonResult getTrackInLot(string equipment)
        {
            var res = EquipmentModels.getLotInProcess(equipment);

            return Json(res, JsonRequestBehavior.AllowGet);
        }

        [HttpGet]
        public JsonResult getTrackInUser(string equipment)
        {
            var res = EquipmentModels.getTrackInUser(equipment);

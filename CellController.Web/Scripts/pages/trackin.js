    ...
    $.ajax({
        ...
        if (result.ContainerName != null && result.ContainerName != "") {
            ...
            if (type.toUpperCase() == "LASER MARK TEST") {
                ...
                getRecipeInfo(function (output) {

                    if (output == true) {
                        notification_modal_confirm_SMT();
                    }
                    else {
                        notification_modal_dynamic("Notification", "Invalid Recipe", 'danger', identifier);
                        identifier++;
                    }
...
function ProceedTrackIn(markReq) {
    ...
    if (lstLoadedRecipe.length == 0 && loadedTemplate != null && loadedTemplate != "")
    {
        arr_program.push(loadedTemplate);
        arr_equip.push(loadedEquip);
    }
    else
    {
        for (var x = 0; x < lstLoadedRecipe.length; x++) {
            arr_program.push(lstLoadedRecipe[x]);
            arr_equip.push(lstLoadedEquip[x]);
        }
    }
        ...
        $.ajax({
            ...
            if (returnval != "" && returnval != null)
            {
                if (returnval.toUpperCase().includes("ERROR"))
                {
                    ...
                    $.ajax({
                        url: '/TrackIn/TrackIn',
                        data: { UserID: operator, LotNo: lotNo, Equipment: equipment, TrackInQty: quantity, Comment: comment, Location: location, lstProgram: arr_program, lstEquip: arr_equip },
                        method: 'POST'
...
//build html modal track in confirmation (SMT)
function notification_modal_confirm_SMT() {
    ...
    modal += '<div class="modal-footer" style="text-align:center;">';
    modal += '<button type="button" class="btn btn-success" onclick="InitSECSGEM();">OK</button>';
    modal += '<button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>';
...
function InitSECSGEM() {
    ...
    //equipment = 'SP-002_DEK';
    //equipment = 'OVN-002_HLLER';
    //equipment = 'CM-002_PANASONIC';

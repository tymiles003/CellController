    ...
    try {
        if (isSignalR == "True") {
            ...
            $.connection.hub.start().done(function () {
                ...
                if (isSignalR == "True") {
                    notification_modal_signalr("SignalR Error", "Can't connect to SignalR Server", "danger", identifier_temp);
                    identifier_temp++;
                }
            });
...
function proceed(equipment, type) {
    ...
    $.ajax({
        ...
        beforeSend: function (xhr) {
            notification_modal_dynamic_loading("Notification", 'Processing Request. Please wait.', 'success', identifier_temp);
            temp_identifier = identifier_temp;
            identifier_temp++;
        }
        ...
        if (opcResult != null) {
            ...
            if (opcResult.Error == false) {
                ...
                notification_modal_dynamic("Notification", label, 'success', identifier_temp);
                identifier_temp++;
            }
            else {
                label = 'Send Command to Machine:&nbsp;<label style="color:red;">Failed</label>';

                parentModal = "modal_div" + identifier_temp;
                ...
                notification_modal_dynamic("Notification", label, 'warning', identifier_temp);
                identifier_temp++;
            }
        }
        else {
